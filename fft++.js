...

{
  type:'input',
  name:'pck',
  message:'[>] Insert Username ( check Followers ) :',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
const piceksu = async function(pck){
  const url = 'https://www.instagram.com/'+pck+'/'
  const option = {
    url: url,
    method: 'GET'
  }
  try{
    ...
    if (json.entry_data.ProfilePage[0].graphql.user.is_private) {
      ...
      const id = json.entry_data.ProfilePage[0].graphql.user.id;
      const followers = json.entry_data.ProfilePage[0].graphql.user.edge_followed_by.count;
      const following = json.entry_data.ProfilePage[0].graphql.user.edge_follow.count;
      return Promise.resolve({id,followers,following});
    }
  } catch (err){
    return Promise.reject(err);
  }
...
.then(answers => {
  ...
  },answers.pck,answers.target,answers.username,answers.sleep,answers.mysyntx,);
})
