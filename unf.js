...
const Excute = async function(User,sleep,Usernames){
    ...
    try {
        ...
        do{
            ...
            getPollowers = _.chunk(getPollowers, 12);
            for (let i = 0; i < getPollowers.length; i++) {
              var timeNow = new Date();
                ...
                await Promise.all(getPollowers[i].map(async(account) => {
                    ...
                    console.log(chalk`[{bold.red [#]Akun FFT : ${Usernames}}] Unfollow {yellow @${account.params.username}} => ${doUnfollow}`);
                }));
...
inquirer.prompt(User)
    .then(answers => {
        ...
        },answers.sleep,answers.username);
    })
